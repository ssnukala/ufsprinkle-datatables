...
//SnUtil::logarr($var_retval,"Line 192 Datatable dadta ");    
...
//logarr($par_coldef,"Line 321 columne defs");
        
        if (isset($request['search']) && $request['search']['value'] != '') {
            $str = $request['search']['value'];

//SnUtil::logarr($request['search'],"Line 283 search array is value is { $str }"); 
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                $requestColumn = $request['columns'][$i];
                $columnIdx = array_search($requestColumn['data'], $dtColumns);
                $column = $par_coldef[$columnIdx];

//SnUtil::logarr($requestColumn,"Line 341 checking for Searchagble == 'Y')= (value ".$requestColumn['searchable'].") Type is ".(gettype($requestColumn['searchable'])));                
...
//SnUtil::logarr($globalSearch,"Line 350 global serach array");
